...
`+JSON.stringify(o,null,2):p=" "+this.formatData(o));let m="";if(n){let{sessionId:E,memorySessionId:T,correlationId:O,..._}=n;Object.keys(_).length>0&&(m=` {${Object.entries(_).map(([h,y])=>`${h}=${y}`).join(", ")}}`)}let g=`[${i}] [${d}] [${a}] ${l}${s}${m}${p}`;if(this.logFilePath)try{(0,L.appendFileSync)(this.logFilePath,g+`
